...
- `#workspaceUndo`, `#workspaceRedo`, `#zoomOut`, `#zoomIn`, `#zoomFit`, `#workspaceToggleBottom[data-panel-command="bottom"]`, and the labeled `.workspace-run-button[data-bottom-tab="machine"]` are the visible workspace toolbar commands, in that order (`#showCodeFromWorkspace` follows Run). `#showToolboxFromWorkspace` and `#showCodeFromWorkspace` remain in the same toolbar as hidden responsive restore controls. `#zoomLabel` remains as a visually-hidden state/readout target. **Resolved (post-review):** the header's redundant `.header-run-button` was removed (the workspace toolbar's Run is now the only Run control); `#workspaceToggleBottom` is new and shares `data-panel-command="bottom"` with `#toggleVizDock`, kept in `aria-pressed` sync by `setVisualizationOpen`.
- `#clearWorkspace` moved to Header → File; `#presentationMode` moved to Header → View. Their IDs, click targets, ARIA state updates, and keyboard routes are unchanged. **Resolved (post-review):** `#toggleVizDock` remains in Header → View but is no longer the only "Toggle Bottom Panel" control — see `#workspaceToggleBottom` above.
- Settings controls live in `#settingsDialog`, labelled by `#settingsDialogTitle` and opened by `#openSettings[data-activity="settings"]`; `#closeSettingsDialog` closes it and returns focus to its invoker. The theme (segmented control) and autosave/perspective IDs remain unique and unchanged. **Resolved (post-review):** the renderer picker (`#blocklyThemeMenuButton`/`#blocklyThemeSubMenu`) moved out of Settings' "Block appearance" section (now removed) into its own top-level header "Renderer" menu — see the "Header refactor update" note below. A header quick-access theme toggle (`#themeToggleButton`) was also added, independent of and in addition to Settings → Color theme.
...
## Header refactor update (post-review)

- `.header-run-button` (a second, redundant Run control) has been removed from Header → menu bar. The workspace toolbar's `.workspace-run-button[data-bottom-tab="machine"]` is now the sole Run control; no ID or route was lost, since it dispatched through the same `data-bottom-tab="machine"` delegated handler.
- `#topbarFileName` moved from the header's `.brand-copy` into the status bar (`.statusbar-left`, first item). The ID, `aria-live="polite"`, and the `setWorkspaceTitle()` write contract in `block_lambda.ts` are unchanged — only its DOM position and a new `#topbarFileName { max-width; overflow; text-overflow }` presentation rule are new.
- `#themeToggleButton` is a new always-visible icon button, a direct child of `<header class="topbar">` (a sibling of `#topbarActions`, not inside it), so it stays reachable at every viewport including the compact hamburger breakpoints. It dispatches to the same `toggleTheme()`/`#themeToggle` checkbox contract already used by the command-palette "Preferences: Toggle Color Theme" entry, and its icon/`aria-pressed`/`aria-label` are kept in sync by `syncThemeControls()`.
- `#blocklyThemeMenuButton`/`#blocklyThemeSubMenu` moved from Settings → "Block appearance" (section removed) into a new top-level `.renderer-menu-wrapper` in the header menu bar, between View and More. The IDs, `data-blockly-renderer` values, `aria-checked` sync, and all keyboard/outside-click/Escape handling in `installBlocklyThemeMenu()` are unchanged — only the DOM location and the wrapper class changed. The wrapper is deliberately `.renderer-menu-wrapper`, not `.app-menu` (the same pattern already used by `.example-menu-wrapper`), so the generic per-`.app-menu` click-wiring loop in `workbench.ts` does not also attach a second, competing open/close listener to the same trigger button. The individual renderer options changed `role="radio"` (inside a `role="radiogroup"`) to `role="menuitemradio"` (inside a `role="menu"`), the correct ARIA pattern now that the container is a menu rather than a settings disclosure; this is a markup-only change with no JS dependency on the old role value.
- `workbench.ts`'s Escape-key priority chain gained an explicit `#blocklyThemeSubMenu` case (mirroring the existing `#examplesSubMenu` case) positioned before the generic `.app-menu-popup` fallback, and lost the old `settingsDialog?.open` inner check for the renderer submenu (dead code now that the renderer menu is not nested inside Settings).
...
| Resizers | `sidebarResizeHandle`, `resizeHandle`: vertical separator, disabled (`tabindex="-1"`, `aria-disabled="true"`) in the ≤1240px side-panel overlay drawers; `vizResizer`: horizontal separator, focusable with value min/max/now at every viewport (**resolved, post-review**: previously also disabled at ≤620px — the bottom panel is not converted to a side-panel-style overlay drawer at that width, only repositioned via `position:fixed`, so disabling it was a reachability bug, not an intentional drawer-mode exception) | Must preserve exactly | Keyboard resize and announced value |
...
| `vizResizer` | May move but must retain identity | Pointer drag; ArrowUp/Down; 180px–72vh live range; manual resize clears maximized state; enabled (`tabindex=0`, `aria-disabled=false`) at every viewport, including the ≤620px bottom drawer (**resolved, post-review** — see the Resizers row above); the grid row it occupies is 14px at ≤1240px (was a flat 8px) for an easier touch/pointer target |
