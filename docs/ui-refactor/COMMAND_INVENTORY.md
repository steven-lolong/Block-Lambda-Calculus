...
The workspace toolbar contains Undo, Redo, Zoom out, Zoom in, Fit workspace, `workspaceToggleBottom` (Toggle Bottom Panel), the labeled primary Run action, and `showCodeFromWorkspace`. `presentationMode` moved to Header → View, and `clearWorkspace` moved to Header → File. The hidden responsive restore affordance `showToolboxFromWorkspace` remains in the workspace header because it is a required drawer control, not a routine command. **Resolved (post-review):** the header previously also carried a second, redundant Run button (`.header-run-button`); it has been removed, leaving the workspace toolbar's Run as the sole Run control. `workspaceToggleBottom` and `toggleVizDock` (Header → View) both carry `data-panel-command="bottom"` and are kept in `aria-pressed` sync by `setVisualizationOpen`.
...
| Open/close menu or submenu | Disclose File, Examples, View, Renderer, or More choices | Header menu bar | `examplesMenuButton`, `examplesSubMenu`, `blocklyThemeMenuButton`, `blocklyThemeSubMenu`; other top-level triggers have no IDs | ArrowDown enters a menu; ArrowUp/Down and Home/End navigate Examples and Renderer; Escape/outside click closes | No | Header | None | No | High: focus navigation, popup ancestry, `hidden`, and `aria-expanded` are coupled. **Resolved (post-review):** Renderer is now its own top-level header menu (previously a disclosure nested in Settings → Block appearance, which no longer exists); `blocklyThemeMenuButton`/`blocklyThemeSubMenu` kept their IDs and behavior, only their DOM location and wrapper (`.renderer-menu-wrapper`, not `.app-menu`, to avoid double-wiring the generic menu-open loop) changed. |
...
| Select Tude renderer | Reinject Blockly with the project renderer | Header → Renderer | `[data-blockly-renderer="tude"]`, `blocklyThemeMenuButton`, `blocklyThemeSubMenu` | None | No | Header → Renderer | None | No | Very high: workspace disposal/reinjection and connector geometry. **Resolved (post-review):** moved out of Settings → Block appearance (removed) into its own top-level header menu, per user request that a dedicated Renderer menu was missing. |
| Select Zelos renderer | Reinject Blockly with Zelos | Header → Renderer | `[data-blockly-renderer="zelos"]` | None | No | Header → Renderer | None | No | High |
| Select Thrasos renderer | Reinject Blockly with Thrasos | Header → Renderer | `[data-blockly-renderer="thrasos"]` | None | No | Header → Renderer | None | No | High |
...
| Show Run activity | Legacy activity state; runtime routes now open bottom tools | Hidden compatibility state | `[data-activity="run"]`, `[data-sidebar-view="run"]` | None | Run palette commands | Workspace Run | Bottom runtime tabs and palette | No visible duplicate | Medium |
...
| Toggle bottom panel | Open/close bottom tools | Header → View; workspace toolbar; bottom close button | `toggleVizDock`, `workspaceToggleBottom`, `vizCollapse`, `[data-panel-command="bottom"]` | Ctrl/Cmd+J | Yes — `View: Toggle Bottom Panel` | Header → View | Workspace toolbar, palette, and shortcut | Yes, close is contextual | Very high: `data-open`, mobile drawer, Blockly resize. **Resolved (post-review):** `workspaceToggleBottom` added to the workspace toolbar (right before Run) per user request; both it and `toggleVizDock` share `data-panel-command="bottom"` and are kept in `aria-pressed` sync by `setVisualizationOpen`. |
...
| Open/run CEK machine | Open machine and auto-load first program | Workspace Run; Bottom → Semantics → CEK machine | `[data-bottom-tab="machine"]`, runtime `bottomTab-machine`, `.workspace-run-button` | Arrow/Home/End in Semantics tablist | Yes — `Run: CEK Machine` | Workspace toolbar Run | Semantics and palette | No visible duplicate | Very high: current language-default primary route. **Resolved (post-review):** the redundant `.header-run-button` was removed; the workspace toolbar's Run is now the sole Run control. |
...
| Evaluate application — CbS | Open trace for the clicked application | Application context menu | Registry ID `lambdaVizCallByStructure` | Context-menu dependent | No; generic CbS palette command uses top-level program | Block context menu | Palette | Yes by behavior, different target | Very high |
| Evaluate application — CbV | Open trace for the clicked application | Application context menu | Registry ID `lambdaVizCallByValue` | Context-menu dependent | No; generic CbV palette command uses top-level program | Block context menu | Palette | Yes by behavior, different target | Very high |
...
| Tools top-level menu | Examples and renderer selection | Labelled Examples menu; labelled Renderer menu (Header → Renderer) |
...
| Header theme switch | Light/dark theme | Header quick-toggle button (`themeToggleButton`); Settings → Color Theme; command palette toggle |
| Status file item | Current file/project context | Status bar (`topbarFileName`) |
...
**Resolved (post-review, this pass):** four further issues were reported against the completed refactor and fixed:

1. The header's `.header-run-button` duplicated the workspace toolbar's Run control; it has been removed, leaving one Run surface.
2. `showCodeFromWorkspace` moved to after Run in the workspace toolbar (was before it).
3. A `workspaceToggleBottom` button was added to the workspace toolbar, positioned before Run, toggling the same state as `toggleVizDock`.
4. `topbarFileName` moved from the header brand area to the status bar (same ID, same `setWorkspaceTitle()` contract, new DOM position).
5. A header quick-access theme toggle (`themeToggleButton`, sun/moon icon) was added; Settings → Color Theme is unchanged and still works.
6. The renderer picker (`blocklyThemeMenuButton`/`blocklyThemeSubMenu`) moved out of Settings → Block appearance (now removed) into its own top-level header "Renderer" menu, styled like the other header menus but kept structurally independent (`.renderer-menu-wrapper`, not `.app-menu`) so its bespoke open/close/keyboard logic in `installBlocklyThemeMenu()` is not double-wired by the generic `.app-menu` loop.
7. The bottom-panel resizer (`vizResizer`) is no longer disabled at ≤620px; it is now enabled at every viewport, and its grid row is 14px (was 8px) at ≤1240px for an easier touch/pointer target.
