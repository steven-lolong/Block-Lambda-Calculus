...
The current-location columns reflect the completed header/status-bar refactor. Future-location columns remain guidance for later panel work.
...
| Open workspace | Pick and deserialize a `.blc` file | File menu; Project sidebar | `loadWorkspace` plus proxies using `[data-command-target]` | Ctrl/Cmd+O | Yes — `File: Open Workspace…` | Header → File | Palette and shortcut | Yes | High: file picker, serialization, autosave update |
| Save workspace as | Name, serialize, and download a `.blc` file | File menu; Project sidebar | `saveWorkspace`, `saveNameDialog`, `saveNameInput` plus proxies | Ctrl/Cmd+S | Yes — `File: Save Workspace As…` | Header → File | Palette and shortcut | Yes | High: dialog and browser download behavior |
...
| Toggle light/dark theme | Switch shell and Blockly theme | Settings sidebar | `themeToggle` | None | Yes — `Preferences: Toggle Color Theme` | Settings → Color Theme | Palette | No | High: persistence, browser theme color, Blockly rerender |
| Select dark theme | Select the dark palette explicitly | Settings sidebar | `themeToggle`, `[data-theme-mode="dark"]` | None | Toggle entry reaches the same state | Settings → Color Theme | Palette toggle | No | Medium |
| Select light theme | Select the light palette explicitly | Settings sidebar | `themeToggle`, `[data-theme-mode="light"]` | None | Toggle entry reaches the same state | Settings → Color Theme | Palette toggle | No | Medium |
...
| Select Tude renderer | Reinject Blockly with the project renderer | Settings → Block Appearance | `[data-blockly-renderer="tude"]`, `blocklyThemeMenuButton`, `blocklyThemeSubMenu` | None | No | Settings → Block Appearance | None | No | Very high: workspace disposal/reinjection and connector geometry |
| Select Zelos renderer | Reinject Blockly with Zelos | Settings → Block Appearance | `[data-blockly-renderer="zelos"]` | None | No | Settings → Block Appearance | None | No | High |
| Select Thrasos renderer | Reinject Blockly with Thrasos | Settings → Block Appearance | `[data-blockly-renderer="thrasos"]` | None | No | Settings → Block Appearance | None | No | High |
...
| Show Settings activity | Open settings sidebar | Activity bar; Header → More | `[data-activity="settings"]`, `[data-sidebar-view="settings"]` | None | No | Activity bar | More | Yes, permitted secondary entry | Medium |
...
| Run type analysis / show Types | Open bottom Types tab (analysis itself is kept current elsewhere) | More menu; bottom tab | `[data-bottom-tab="types"]`, runtime `bottomTab-types` | Arrow/Home/End in bottom tabs | Yes — `View: Show Inferred Types` | Bottom panel → Types | More and palette | Yes | Medium |
| Show Problems panel | Open bottom type-inference diagnostics | More menu; Problems sidebar footer; bottom tab; palette | runtime `bottomTab-problems`; `[data-bottom-tab="problems"]` | Arrow/Home/End in bottom tabs | Yes — `View: Show Problems` | Bottom panel → Problems | Status bar, More, and palette | Yes | High: counts and two diagnostics surfaces |
...
| Command name | Purpose | Current visible locations | Current element IDs | Keyboard shortcut | Command-palette entry | Primary future location | Secondary permitted location | Whether it is duplicated | Migration risk |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Run Call-by-Structure trace | Open/render the default substitution strategy | More menu; Run sidebar; bottom tab; block context menu for a specific application | `[data-bottom-tab="structure"]`, runtime `bottomTab-structure` | Arrow/Home/End in tabs | Yes — `Run: Call-by-Structure Trace` | Bottom Semantics | More, palette, context menu | Yes | Very high: selected/top-level program distinction |
| Run Call-by-Value trace | Open/render value-first substitution | More menu; Run sidebar; bottom tab; application context menu | `[data-bottom-tab="value"]`, runtime `bottomTab-value` | Arrow/Home/End | Yes — `Run: Call-by-Value Trace` | Bottom Semantics | More, palette, context menu | Yes | Very high |
| Open/run CEK machine | Open machine and auto-load first program | Primary header Run; Run sidebar; bottom tab | `[data-bottom-tab="machine"]`, runtime `bottomTab-machine` | Arrow/Home/End | Yes — `Run: CEK Machine` | Header Run | Run sidebar, bottom tab, and palette | Yes | Very high: current language-default primary route |
| Open/run Lockstep debugger | Open lockstep and auto-load first program | More menu; Run sidebar; bottom tab | `[data-bottom-tab="stepper"]`, runtime `bottomTab-stepper` | Arrow/Home/End | Yes — `Run: Lockstep Debugger` | Bottom panel → Semantics/runtime | More and palette | Yes | Very high |
...
1. Project-sidebar Open/Save/Recover versus File; retain File and make the project view informational in a later panel step.
2. Run-sidebar copies of CbS/CbV/CEK/Lockstep versus primary Run, More, and bottom tabs.
3. View-menu perspective presets versus Settings; Presentation remains appropriate in View because F11 is a view mode.
4. Workspace custom zoom buttons versus Blockly-injected zoom controls.
5. Multiple sidebar/code/bottom close/restore buttons are responsive counterparts and require compact/phone coverage before consolidation.
